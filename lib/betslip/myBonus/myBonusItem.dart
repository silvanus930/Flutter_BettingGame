...
class MyBonusItem extends StatefulWidget {
  final bonusData;
  final activeBonus;

  const MyBonusItem({Key? key, this.bonusData, this.activeBonus})
      : super(key: key);
  @override
  State<MyBonusItem> createState() => MyBonusItemState();
}

class MyBonusItemState extends State<MyBonusItem> {
  bool showMore = false;
  Map bonusLimits = {};
  String timeFormat = "";
  ...
  useBonus(bonus) async {
    if (widget.activeBonus.isNotEmpty &&
        widget.activeBonus['accountBonusCampaignId'] ==
            bonus['accountBonusCampaignId']) {
      StoreProvider.of<AppState>(context)
          .dispatch(SetActiveBonusAction(activeBonus: {}));
    } else {
      if (bonusLimits.isEmpty) {
        await getBonusConditions();
      }
      if (bonusLimits.isNotEmpty) {
        bonus['ticketTypes'] = bonusLimits['ticketTypes'];
        bonus['minOdds'] = bonusLimits['minOdds'];
        bonus['minTotalOdds'] = bonusLimits['minTotalOdds'];
        bonus['minTipps'] = bonusLimits['minTipps'];
        bonus['minStake'] = bonusLimits['minStake'];
        bonus['mustOnce'] = bonusLimits['mustOnce'];
      }
      StoreProvider.of<AppState>(context)
          .dispatch(SetActiveBonusAction(activeBonus: bonus));
    }
  }
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                !widget.bonusData['forced'] || !widget.bonusData['blocked']
                    ? TextButton(
                        child: Text(
                            widget.bonusData['accountBonusCampaignId'] ==
                                    widget.activeBonus['accountBonusCampaignId']
                                ? LocaleKeys.unuseBonus.tr()
                                : LocaleKeys.useBonus.tr()),
                        onPressed: () {
                          useBonus(widget.bonusData);
                        },
                      )
                    : SizedBox(),
