...
class MatchDetailsDataForecast extends StatefulWidget {
  final matchData;
  final betDomains;

  const MatchDetailsDataForecast({Key? key, this.matchData, this.betDomains})
      : super(key: key);

  @override
  State<MatchDetailsDataForecast> createState() =>
      _MatchDetailsDataForecastState();
}

class _MatchDetailsDataForecastState extends State<MatchDetailsDataForecast> {
  int first = -1;
  int second = -1;
  List any = [];
  Map oddObject = {};
  ...
  @override
  void didUpdateWidget(MatchDetailsDataForecast oldWidget) {
  ...
  handleObject() {
    if (first != -1 && second != -1) {
      Map odd = {};
      odd = widget.betDomains[0]['odds'].firstWhere(
          (e) => e['runners'][0] == first && e['runners'][1] == second);
      setState(() {
        oddObject = odd;
      });
      ...
      odd = widget.betDomains
          .firstWhere((e) => e['betDomainNumber'] == 2021)['odds']
          .firstWhere((e) =>
              e['runners'].contains(any[0]) && e['runners'].contains(any[1]));
      setState(() {
        oddObject = odd;
      });
    } else {
  ...
  Widget _showCompetitorsTable(betDomains) {
    ...
    bool reverseForecast =
        widget.betDomains.any((e) => e['betDomainNumber'] == 2021);

    betDomains[0]['odds']
        .sort((a, b) => a["value"].compareTo(b["value"]) as int);

    sortCompetitors(betDomains[0]['odds']).forEach((e) {
      ...
      rows.add(DataRow(cells: [
        ...
        DataCell(widget.matchData['sport']['sportId'] == 100
            ...
            : Text('${competitor['competitor']['defaultName']}',
                style: TextStyle(fontSize: 9))),
        DataCell(InkWell(
            onTap: second == e['number'] || any.length > 0
                ? null
                : () {
                    handleClickPlace("first", e['number']);
                  },
            child: Opacity(
                opacity: second == e['number'] || any.length > 0 ? 0.3 : 1,
                child: Container(
        ...
        DataCell(InkWell(
            onTap: first == e['number'] || any.length > 0
                ? null
                : () {
                    handleClickPlace("second", e['number']);
                  },
            child: Opacity(
                opacity: first == e['number'] || any.length > 0 ? 0.3 : 1,
                child: Container(
                  alignment: Alignment.center,
                  height: 40,
                  width: MediaQuery.of(context).size.width * 0.12,
        ...
        if (reverseForecast)
          DataCell(InkWell(
              onTap: first != -1 ||
                      second != -1 ||
                      any.length == 2 && !any.contains(e['number'])
                  ? null
                  : () {
